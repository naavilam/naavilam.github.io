---
layout: home
title: 
date: 2020-11-20 
---
### Hi, I'm Nara
I hold a Bachelor’s degree in Computer Engineering, with professional experience in software engineering. I am a Physics major candidate, with current research focused on the analysis of data from heavy-ion experiments at the Large Hadron Collider (LHC) and RHIC, leveraging advanced statistical frameworks to better understand the universe in its earliest stages, when matter existed as a quark–gluon plasma.
<!-- Dumbarton is a Jekyll Theme developed by [Tyler Butler](https://github.com/tcbutler320). The theme is designed for academics and features a simple home page with an about me section and an interactive highlights section to describe publications, coursework, courses taught, and projects. You can checkout the project on [Github](https://github.com/tcbutler320/Jekyll-Theme-Dumbarton), feel free to make contributions and feature requests and don't forget to give it a star. -->

<a href="/about.html" class="highlighted">More Information About Me</a>